(function () {
  ...
  function applyAccessibleHover(element, normalFg, normalBg, hoverFg, hoverBg) {
    // Links/buttons are handled by the dedicated hover+11 enhancer.
    // Avoid attaching additional hover listeners / `.ai-hover` class injection that can conflict.
    try {
      const _t = element && element.tagName ? element.tagName.toLowerCase() : "";
      const _isBtn = _t === "button" || (element && element.getAttribute && element.getAttribute("role") === "button");
      const _isLink = _t === "a";
      if (_isBtn || _isLink) {
        return;
      }
  ...
  function applyContrastFixWithHover(element, correctedColor) {
    ...
    // Priority rule: text-color-only hover correction first.
    // Never inject/override hover background here; preserve site hover background visuals.
    const hoverBg = null;
  ...
  function forceApplyStyle(el, property, value) {
    ...
    // C) STABLE ID: Use single stable ID from data attribute, never create new classes
    let fixId = el.getAttribute('data-ai-colorfix-id');
    if (!fixId) {
      fixId = `fix-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      el.setAttribute('data-ai-colorfix-id', fixId);
    }
    
    // A) MUTATION FILTERING: Mark with operation token (not time-based)
    const operationId = generateOperationId();
    markElementOperation(el, operationId);
    const ownedAttrs = extensionOwnedAttributes.get(el) || new Set();
    ownedAttrs.add('style');
    extensionOwnedAttributes.set(el, ownedAttrs);

    // Build selector using stable ID attribute (more reliable than classes)
    const tagName = el.tagName.toLowerCase();
    const selector = `${tagName}[data-ai-colorfix-id="${fixId}"]`;

    const cssRule = `${selector} { ${property}: ${value} !important; }`;

    // Use insertRule for proper stylesheet manipulation (more reliable than textContent)
    try {
      const sheet = contrastSheet.sheet;
      if (sheet) {
        // Remove existing rule for this element and property if it exists
        const rules = Array.from(sheet.cssRules || []);
        for (let i = rules.length - 1; i >= 0; i--) {
          const rule = rules[i];
          if (rule.selectorText && rule.selectorText.includes(fixId) && rule.style.getPropertyValue(property)) {
            sheet.deleteRule(i);
          }
        }
        // Insert new rule at the beginning for maximum priority
        sheet.insertRule(cssRule, 0);
      } else {
        // Fallback to textContent if sheet is not available
        const ruleExists = contrastSheet.textContent.includes(fixId);
        if (!ruleExists) {
          contrastSheet.textContent = cssRule + '\n' + contrastSheet.textContent;
        } else {
          // Update existing rule
          const lines = contrastSheet.textContent.split('\n');
          const updatedLines = lines.map(line => {
            if (line.includes(fixId) && line.includes(property)) {
              return cssRule;
            }
            return line;
          });
      ...
      // Unmark using operation token (cleanup via microtask)
      Promise.resolve().then(() => {
        unmarkElementOperation(el, operationId);
      });
    } catch (e) {
      console.error(`   🚨 [FORCE APPLY] Failed to inject stylesheet rule:`, e);
      unmarkElementOperation(el, operationId);
      return false;
    }
...
// ----------  END REWRITTEN ORIGINAL BLOCK ----------
  ...
  function applyColorWithImportant(el, property, value) {
    ...
    // A) MUTATION FILTERING: Mark element with operation token (not time-based)
    const operationId = generateOperationId();
    markElementOperation(el, operationId);
    const ownedAttrs = extensionOwnedAttributes.get(el) || new Set();
    ownedAttrs.add('style');
    ...
    if (valueMatch && computedMatch) {
      ...
      if (!matches) {
        // Log apply failure once per element per session
        if (!loggedApplyFailure.has(el)) {
          loggedApplyFailure.add(el);
        ...
        // C) STABLE ID: Use single stable ID, don't create new classes
        // Get or create stable fix ID
        let fixId = el.getAttribute('data-ai-colorfix-id');
        if (!fixId) {
          fixId = `fix-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
          el.setAttribute('data-ai-colorfix-id', fixId);
        ...
        // Use forceApplyStyle which uses the stable ID (it will handle its own operation token)
        forceApplyStyle(el, property, value);
        ...
        // Success: colors match - no need to log (reduce console noise)
      }
    } else if (!valueMatch) {
      ...
      // Success - no need to log (reduce console noise)
    } else {
    ...
    // Unmark element after modification completes (operation token cleanup)
    cleanup();
  }
  ...
  // ---------------------------------------------------------------------------
  // Hover-only contrast enhancement (links + buttons)
  // - Applies ONLY during hover events
  // - Restores exact original inline state on hover-out (no residue)
  // - Avoids !important unless it already existed inline for that property
  // ---------------------------------------------------------------------------
  const _AI_HOVER_ENHANCE_DEBUG = true; // easy to disable later
  const _AI_HOVER_BIND_VERSION = 2; // bump when hover binding logic changes

  function _aiHoverLog(msg) {
    if (!_AI_HOVER_ENHANCE_DEBUG) return;
    // Use warn so it's visible even if console filters are set to show only warnings/errors.
    console.warn(msg);
  }

  function _aiHoverEnhanceIdentifier(el) {
    const tag = (el && el.tagName ? el.tagName.toLowerCase() : "unknown");
    const cls = el && el.classList && el.classList.length
      ? "." + Array.from(el.classList).slice(0, 3).join(".")
      : "";
    return `${tag}${cls}`;
  }

  function _aiIsLinkOrButton(el) {
    if (!el || !el.tagName) return false;
    const tag = el.tagName.toLowerCase();
    return tag === "a" || tag === "button" || el.getAttribute("role") === "button";
  }

  function _aiGetNormalBgRgbForHover(el) {
    // Prefer stored corrected normal BG if present (this is the "normal" state after scan).
    ...
    // Next, prefer any corrected BG stored by other hover systems.
    const correctedBg = el.getAttribute("data-corrected-bg");
    if (correctedBg) {
      const parsed = parseCSSColorToRGBA(correctedBg, null);
      if (parsed && parsed[3] > 0.05) return parsed.slice(0, 3);
    }
    // Otherwise use the element's computed background if solid enough.
    const cs = getComputedStyle(el);
    const bg = parseCSSColorToRGBA(cs.backgroundColor, [0, 0, 0, 0]);
    if (bg && bg[3] > 0.05) return bg.slice(0, 3);
    // Fall back to effective background behind the element.
    const eff = getEffectiveBackgroundRGBA(el);
    return eff ? eff.slice(0, 3) : [255, 255, 255];
  }

  function _aiGetNormalFgRgbForHover(el) {
    ...
    const correctedFg = el.getAttribute("data-corrected-fg");
    if (correctedFg) {
      const parsed = parseCSSColorToRGBA(correctedFg, null);
      if (parsed) return parsed.slice(0, 3);
    }
    const cs = getComputedStyle(el);
    const fg = parseCSSColorToRGBA(cs.color, [0, 0, 0]);
    return fg.slice(0, 3);
  }

  function _aiInlineStyleSnapshot(el, prop) {
    return {
      value: el.style.getPropertyValue(prop) || "",
      priority: el.style.getPropertyPriority(prop) || "",
      had: !!(el.style.getPropertyValue(prop) && el.style.getPropertyValue(prop).trim() !== ""),
    };
  }

  function _aiSetInlineRespectingExistingImportant(el, prop, value) {
    // CRITICAL FIX: Always use !important for hover enhancement styles
    // Without !important, site CSS rules (especially those with !important) 
    // can override our hover styles, causing the enhancement to fail silently.
    // The previous logic only used !important if the existing inline style had it,
    // but hover backgrounds often have no prior inline style while site CSS does.
    el.style.setProperty(prop, value, "important");
  }

  function _aiApplyHoverContrastEnhancement(el, threshold = 11) {
    if (!_aiIsLinkOrButton(el)) return { applied: false, reason: "not-target" };

    // SKIP CHECK: Don't apply hover enhancement to elements on image/video backgrounds
    // For hover, we need to check the NORMAL state background (not hover state)
    // because we're enhancing the hover state, so we need to know if the normal state has a solid background
    // Use stored normal background from scan, or check computed style (which might be hover state, so check parent too)
    try {
      // First, try to get stored normal background from scan
      const storedNormalBg = el.getAttribute("data-ai-normal-bg");
      let hasSolidBg = false;
      let hasImageInStyle = false;
      
      if (storedNormalBg) {
        // We have stored normal background - check if it's solid
        const parsed = parseCSSColorToRGBA(storedNormalBg, null);
        if (parsed && parsed[3] >= 0.95) {
          hasSolidBg = true;
        }
      } else {
        // No stored background - check computed style (might be hover state, so also check parent)
        const cs = getComputedStyle(el);
        const elBg = parseCSSColorToRGBA(cs.backgroundColor, [0, 0, 0, 0]);
        hasSolidBg = elBg && elBg[3] >= 0.95;
        
        // Check if element has image background
        const elBgImage = cs.backgroundImage;
        hasImageInStyle = elBgImage && elBgImage !== 'none' && elBgImage.includes('url(') && !elBgImage.includes('gradient');
        
        // If element doesn't have solid bg and doesn't have image, check ancestors
        // Check up to 4 levels to find solid backgrounds in headers/menus
        if (!hasSolidBg && !hasImageInStyle) {
          let checkAncestor = el.parentElement;
          let checkDepth = 0;
          while (checkAncestor && checkAncestor !== document.body && checkDepth < 4) {
            try {
              const ancestorCs = getComputedStyle(checkAncestor);
              const ancestorBgImage = ancestorCs.backgroundImage;
              // Stop if ancestor has image background (no solid color covering it)
              if (ancestorBgImage && ancestorBgImage !== 'none' && ancestorBgImage.includes('url(') && !ancestorBgImage.includes('gradient')) {
                break;
              }
              const ancestorBg = parseCSSColorToRGBA(ancestorCs.backgroundColor, [0, 0, 0, 0]);
              if (ancestorBg && ancestorBg[3] >= 0.95) {
                // Found solid background - safe to apply hover enhancement
                hasSolidBg = true;
                break;
              }
            } catch (e) { }
            checkAncestor = checkAncestor.parentElement;
            checkDepth++;
          }
        }
      }
      
      // For interactive elements (links/buttons), be more permissive
      // If we can't clearly determine background, still try hover enhancement
      // It will fail gracefully if there's truly no solid background
      if (hasImageInStyle) {
        // Element itself has image background - definitely skip
        return { applied: false, reason: "image-background" };
      }
      
      if (!hasSolidBg) {
        // No solid background found - for interactive elements, still try
        // The enhancement logic will check again and fail gracefully if needed
        // This handles cases where background detection is uncertain
        // (e.g., when hovering, computed styles might show hover state)
      }
      // Safe to proceed with hover enhancement
    } catch (e) {
      // Error checking background - for interactive elements, still try hover enhancement
      // It will fail gracefully if there's truly no solid background
    }

    // Get normal background - prefer stored value to avoid hover state contamination
    let normalBg = _aiGetNormalBgRgbForHover(el);
    const normalFg = _aiGetNormalFgRgbForHover(el);
    
    // If we got hover background instead of normal (no stored value), try to get normal from parent
    // When hovering, getComputedStyle returns hover styles, so we need to check parent's normal state
    if (!el.getAttribute("data-ai-normal-bg") && !el.getAttribute("data-corrected-bg")) {
      // No stored normal background - try to get it from parent's computed style
      // Parent's computed style should show normal state even when child is hovered
      const parent = el.parentElement;
      if (parent && parent !== document.body) {
        try {
          const parentCs = getComputedStyle(parent);
          const parentBg = parseCSSColorToRGBA(parentCs.backgroundColor, [0, 0, 0, 0]);
          if (parentBg && parentBg[3] >= 0.95) {
            // Use parent's background as normal background
            normalBg = parentBg.slice(0, 3);
          }
        } catch (e) { }
      }
    }

    // Compute current hover state (as-is, before we change anything).
    // When hovering, getComputedStyle returns hover styles (site's CSS hover state)
    const cs = getComputedStyle(el);
    const hoverFg = parseCSSColorToRGBA(cs.color, [0, 0, 0]).slice(0, 3);
    const bgParsed = parseCSSColorToRGBA(cs.backgroundColor, [0, 0, 0, 0]);
    const hoverBg = (bgParsed && bgParsed[3] > 0.05)
      ? bgParsed.slice(0, 3)
      : normalBg;

    const currentContrast = wcagContrast(hoverFg, hoverBg);
    if (currentContrast >= threshold) {
      // Contrast already meets threshold - no enhancement needed
      // Return silently to prevent flicker (no logging, no DOM operations)
      return { applied: false, reason: "ok", contrast: currentContrast };
    }

    // Snapshot original inline styles once (only if we're going to apply changes).
    if (!el._aiHoverEnhanceRestore) {
      el._aiHoverEnhanceRestore = {
        color: _aiInlineStyleSnapshot(el, "color"),
        bg: _aiInlineStyleSnapshot(el, "background-color"),
      };
    }

    // NEVER apply background changes on hover - only adjust text color
    // Find the best text color that meets contrast threshold against the existing normal background
    let chosenFg = hoverFg;
    let finalContrast = currentContrast;
    const white = [255, 255, 255];
    const black = [0, 0, 0];

    // Try different text colors against the normal background (no background changes)
    // Try white first (works well on dark backgrounds)
    let c = wcagContrast(white, normalBg);
    if (c >= threshold) {
      chosenFg = white;
      finalContrast = c;
    } else {
      // White didn't work, try current hover FG
      c = wcagContrast(hoverFg, normalBg);
      if (c >= threshold) {
        chosenFg = hoverFg;
        finalContrast = c;
      } else {
        // Try normal FG (scan-corrected)
        c = wcagContrast(normalFg, normalBg);
        if (c >= threshold) {
          chosenFg = normalFg;
          finalContrast = c;
        } else {
          // Last resort: try black (works on light backgrounds)
          c = wcagContrast(black, normalBg);
          if (c >= threshold) {
            chosenFg = black;
            finalContrast = c;
          } else {
            // Cannot meet threshold with text-only change - skip enhancement
            return { applied: false, reason: "text-only-insufficient", contrast: c };
          }
        }
      }
    }

    // Apply hover-only text color change (NO background change)
    const hoverFgStr = `rgb(${chosenFg.join(",")})`;
    _aiSetInlineRespectingExistingImportant(el, "color", hoverFgStr);
    // DO NOT apply background-color - never change backgrounds on hover
    el._aiHoverEnhanceActive = true;

    {
      const id = _aiHoverEnhanceIdentifier(el);
      _aiHoverLog(
        `[HOVER+11] IN ${id} | normalBG=rgb(${normalBg.join(",")}) normalFG=rgb(${normalFg.join(",")}) | hoverFG=${hoverFgStr} | contrast=${finalContrast.toFixed(2)}:1`
      );
    }

    return { applied: true, contrast: finalContrast };
  }

  function _aiRestoreHoverContrastEnhancement(el) {
    if (!el || !el._aiHoverEnhanceActive || !el._aiHoverEnhanceRestore) {
      // No enhancement was applied - return silently to prevent flicker
      // Don't log or perform any DOM operations (getComputedStyle can cause reflow)
      return { restored: false };
    }

    const snap = el._aiHoverEnhanceRestore;

    // Log what we're restoring BEFORE we restore (from snapshot values).
    const id = _aiHoverEnhanceIdentifier(el);
    // Format snapshot values for logging (show actual values being restored to, not computed styles after)
    const bgRestoredStr = snap.bg.had ? snap.bg.value : "(removed)";
    const fgRestoredStr = snap.color.had ? snap.color.value : "(removed)";

    // Restore exact original inline state.
    if (!snap.bg.had) {
      el.style.removeProperty("background-color");
    } else {
      el.style.setProperty("background-color", snap.bg.value, snap.bg.priority);
    }

    if (!snap.color.had) {
      el.style.removeProperty("color");
    } else {
      el.style.setProperty("color", snap.color.value, snap.color.priority);
    }

    delete el._aiHoverEnhanceRestore;
    el._aiHoverEnhanceActive = false;

    // Log snapshot values (what we restored TO), not computed styles after restoration
    _aiHoverLog(`[HOVER+11] OUT ${id} | restoredBG=${bgRestoredStr} restoredFG=${fgRestoredStr}`);
  ...
  function fixButtonHoverState(
    ...
    const tagName = el.tagName.toLowerCase();
    const _aiHoverEnhanceEligible = tagName === "button" || el.getAttribute("role") === "button" || tagName === "a";
    // Legacy hover pipeline uses this flag to avoid background injection; keep it consistent.
    const _aiHoverBgForbidden = _aiHoverEnhanceEligible;
    ...
    if (hasBackground && !_aiHoverBgForbidden) {
    ...
    } else if (hasBackground && _aiHoverBgForbidden) {
      // Buttons/links: NEVER inject or lock hover backgrounds.
      // Preserve the site's hover background visuals; adjust text only if needed.
      const hoverBgCandidate =
        originalHoverBg && Array.isArray(originalHoverBg) && originalHoverBg.length >= 4 && originalHoverBg[3] > 0.5
          ? originalHoverBg.slice(0, 3)
          : correctedBgRGB;

      const hoverContrast = wcagContrast(correctedFgRGB, hoverBgCandidate);
      if (hoverContrast >= target) {
        adjustedHoverFg = correctedFgRGB;
        finalHoverContrast = hoverContrast;
      } else {
        const adjustedResult = adjustColorToContrast(correctedFgRGB, hoverBgCandidate, target, {
          isButton: tagName === "button" || el.getAttribute("role") === "button",
          isLink: tagName === "a",
          elementType: (tagName === "button" || el.getAttribute("role") === "button") ? "button" : (tagName === "a" ? "link" : "text"),
        });
        adjustedHoverFg = (adjustedResult && typeof adjustedResult === "object" && "fg" in adjustedResult) ? adjustedResult.fg : adjustedResult;
        finalHoverContrast = wcagContrast(adjustedHoverFg, hoverBgCandidate);
      }
    } else {
    ...
    let hoverBgStr = null;
    if (hasBackground && !_aiHoverBgForbidden && adjustedHoverBg && adjustedHoverBg.length >= 3) {
    ...
    if (!_aiHoverBgForbidden && hoverBgStr && !hoverBgStr.includes("NaN")) {
    ...
    // IMPORTANT: Do not lock background for links/buttons; it breaks native hover visuals.
    if (shouldStoreBg && normalBgStr && !_aiHoverEnhanceEligible) {
      ...
      console.log(
        `   🔒 Applied corrected normal state immediately - FG: ${normalFgStr}, BG: (effective/site)`
      );
    }
    ...
    const hoverIn = async (event) => {
      // Hover-only 11:1 enhancement for links/buttons (no interference if already compliant).
      if (_aiHoverEnhanceEligible) {
        const res = _aiApplyHoverContrastEnhancement(el, 11);
        if (!res.applied) {
          // Respect existing site hover if it already meets threshold.
          return;
        }
        // Applied enhancement; do not run legacy hover pipeline.
        return;
      }
      ...
      const elementInfo = `${el.tagName} "${(el.textContent || "")
        .trim()
        .substring(0, 30)}"`;
      // Log hover in once per element per session
      if (!loggedHoverIn.has(el)) {
        loggedHoverIn.add(el);
      ...
      if (hasBackground && finalHoverBg && !finalHoverBg.includes("NaN")) {
        ...
        console.log(`   🎨 Hover BG preserved (text-only or invalid hover background)`);
      }
    ...
    const hoverOut = (event) => {
      if (_aiHoverEnhanceEligible) {
        // Defer restore by one frame to prevent flicker
        requestAnimationFrame(() => {
          if (el && document.contains(el)) {
            _aiRestoreHoverContrastEnhancement(el);
          }
        });
        return;
      }
      ...
      const elementInfo = `${el.tagName} "${(el.textContent || "")
        .trim()
        .substring(0, 30)}"`;
      // Log hover out once per element per session
      if (!loggedHoverOut.has(el)) {
        loggedHoverOut.add(el);
    ...
    applyColorWithImportant(el, "color", normalFgStr);
    // Buttons/links: do NOT lock background inline; it breaks native hover visuals.
    if (!_aiHoverBgForbidden && shouldStoreBg && normalBgStr) {
    ...
    try {
      const tagName = el.tagName.toLowerCase();
      const _skipInjectedHoverRuleForTargets =
        tagName === "a" || tagName === "button" || el.getAttribute("role") === "button";

      if (!_skipInjectedHoverRuleForTargets) {
  ...
  // Get all sections sorted by visual vertical position (top to bottom)
  // This ensures we process sections in strict visual order: header → body → footer
  // DETERMINISTIC ORDERING: Sorts by Y-coordinate of bounding rect top edge
  function getSectionsSortedByZIndex() {
    const sections = document.querySelectorAll(
      "section, header, footer, main, div, nav, article, aside"
    );

    const sectionsWithPosition = Array.from(sections)
      .map(sec => {
        try {
          // Get bounding rect for visual position
          const rect = sec.getBoundingClientRect();
          const info = sectionBackgroundCache.get(sec);
          return { 
            section: sec, 
            top: rect.top + window.scrollY, // Absolute Y position from page top
            left: rect.left + window.scrollX, // For secondary sorting (left to right)
            info: info 
          };
        } catch (e) {
          return { section: sec, top: Infinity, left: 0, info: null };
        }
      })
      .sort((a, b) => {
        // DETERMINISTIC: Sort by visual Y-position (top of element), top to bottom
        // Primary: Y-coordinate (vertical position)
        if (Math.abs(a.top - b.top) > 5) { // 5px tolerance for elements on same visual line
          return a.top - b.top;
        }
        // Secondary: X-coordinate (left to right) for elements at same Y
        if (Math.abs(a.left - b.left) > 5) {
          return a.left - b.left;
        }
        // Tertiary: DOM order as final tiebreaker for truly overlapping elements
        const position = a.section.compareDocumentPosition(b.section);
        return position & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1;
      });

    return sectionsWithPosition.map(item => item.section);
  }

  // Get all elements within a specific section, sorted by visual position (top to bottom)
  // DETERMINISTIC ORDERING: Elements are sorted by Y-coordinate for strict top-to-bottom processing
  function getElementsInSection(section) {
    ...
    allElements.forEach(el => {
      ...
      if (isInteractive && el.hasAttribute('data-ai-skip-reason') && el.getAttribute('data-ai-skip-reason') === 'image') {
        // Interactive element marked as image - check if it or immediate ancestor has solid background
        try {
          const elCs = getComputedStyle(el);
          const elBg = parseCSSColorToRGBA(elCs.backgroundColor, [0, 0, 0, 0]);
          let hasSolidBg = elBg[3] > 0.5;
          
          // If element doesn't have solid bg, check immediate ancestors (e.g., <a> inside <li> tab)
          if (!hasSolidBg) {
            let checkAncestor = el.parentElement;
            let checkDepth = 0;
            while (checkAncestor && checkAncestor !== document.body && checkDepth < 5) {
              try {
                const ancestorCs = getComputedStyle(checkAncestor);
                const ancestorBgImage = ancestorCs.backgroundImage;
                // Stop if ancestor has image background
                if (ancestorBgImage && ancestorBgImage !== 'none' && ancestorBgImage.includes('url(') && !ancestorBgImage.includes('gradient')) {
                  break;
                }
                const ancestorBg = parseCSSColorToRGBA(ancestorCs.backgroundColor, [0, 0, 0, 0]);
          ...
          if (hasSolidBg) {
            // Has solid background (own or inherited) - remove skip reason and process it
    ...
    // DETERMINISTIC: Sort elements by visual Y-position (top to bottom)
    // This ensures fixes are applied in strict visual order within each section
    return sectionElements.sort((a, b) => {
      try {
        const rectA = a.getBoundingClientRect();
        const rectB = b.getBoundingClientRect();
        // Primary: Y-coordinate (top to bottom)
        if (Math.abs(rectA.top - rectB.top) > 3) { // 3px tolerance
          return rectA.top - rectB.top;
        }
        // Secondary: X-coordinate (left to right) for elements on same line
        if (Math.abs(rectA.left - rectB.left) > 3) {
          return rectA.left - rectB.left;
        }
        // Tertiary: DOM order as final tiebreaker
        const position = a.compareDocumentPosition(b);
        return position & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1;
      } catch (e) {
        return 0; // Keep original order if getBoundingClientRect fails
      }
    });
  }
  ...
  async function processElementForContrast(
    ...
    // Target for this scan is resolved once in scan initialization (scanWithAI).
    // Never resolve ML availability per element.
    ...
    try {
      ...
      // EXCEPTION: Interactive elements (buttons, links) with solid background coverage
      // should be processed even if distant ancestors have background images.
      // This allows tab links, header nav buttons, etc. to be fixed.
      // Check: element itself OR any immediate ancestor (before image bg) has solid color background.
      const tagNameLower = el.tagName ? el.tagName.toLowerCase() : '';
      const isInteractiveElement = tagNameLower === 'button' || 
                                    tagNameLower === 'a' || 
                                    tagNameLower === 'input' ||
                                    el.getAttribute('role') === 'button' ||
                                    el.getAttribute('role') === 'link';
      
      let skipAncestorCheck = false;
      if (isInteractiveElement) {
        // First check the element itself
        const elBgColor = parseCSSColorToRGBA(cs.backgroundColor, [0, 0, 0, 0]);
        if (elBgColor && elBgColor[3] > 0.5) {
          console.log(`   ✅ [SAFEGUARD] Interactive element ${el.tagName} has solid background (a=${elBgColor[3].toFixed(2)}) - processing despite ancestor images`);
          skipAncestorCheck = true;
        } else {
          // Element doesn't have its own background - check immediate ancestors for solid color bg
          // This handles cases like <a> tags inside <li> tab containers where the <li> has the solid bg
          let checkAncestor = el.parentElement;
          let checkDepth = 0;
          const maxCheckDepth = 5; // Only check close ancestors (immediate container like <li>, <button>, etc.)
          
          while (checkAncestor && checkAncestor !== document.body && checkDepth < maxCheckDepth) {
            try {
              const ancestorStyle = window.getComputedStyle(checkAncestor);
              const ancestorBgImage = ancestorStyle.backgroundImage;
              const ancestorBgColor = parseCSSColorToRGBA(ancestorStyle.backgroundColor, [0, 0, 0, 0]);
              
              // If this ancestor has a background IMAGE, stop checking - can't skip ancestor check
              if (hasBackgroundImageValue(ancestorBgImage)) {
                break;
              }
              
              // If this ancestor has a solid opaque background COLOR, we can skip the ancestor image check
              // because the element is visually on top of this solid color, not the distant image
              if (ancestorBgColor && ancestorBgColor[3] > 0.5) {
                console.log(`   ✅ [SAFEGUARD] Interactive element ${el.tagName} has solid background from ancestor ${checkAncestor.tagName} (a=${ancestorBgColor[3].toFixed(2)}) - processing despite distant ancestor images`);
                skipAncestorCheck = true;
                break;
              }
            } catch (e) {
      ...
      // Skip this check for interactive elements with their own solid backgrounds
      let ancestor = el.parentElement;
      let depth = 0;
      const maxDepth = 20; // Prevent infinite loops
      
      while (!skipAncestorCheck && ancestor && ancestor !== document.body && ancestor !== document.documentElement && depth < maxDepth) {
        try {
    ...
    try {
      ...
      // Always check readability; ML can be used for prediction-only when available for this scan.
      ...
      // CRITICAL FIX: Always extract element context (works on-device now)
        try {
          ...
          // Default readability check is WCAG-only (identical behavior when backend is unavailable)
          try {
            const effectiveTarget = target;
          ...
          // Server-based supervised ML inference (prediction-only), using existing /readability schema.
          // Deterministic call policy (borderline band or autoCorrect about to apply).
          if (mlAvailable) {
            const delta = cr - target;
            const withinLowerBand = cr < target && (target - cr) <= 0.75;
            const withinUpperBand = cr >= target && (cr - target) <= 0.5;
            const shouldCallMl = withinLowerBand || withinUpperBand || (autoCorrect && shouldFlag);

            if (shouldCallMl) {
              stats.totalMlCalls++;

              const baseUrl = window.__aiBackendBaseUrlForScan || null;
              // element_type must be strictly one of: "text", "button", "link"
              // Map DOM elements deterministically (no tag names sent to backend):
              // button -> "button", a -> "link", everything else -> "text"
              const tag = (el && el.tagName ? String(el.tagName).toLowerCase() : "");
              const elementTypeForMl = tag === "button" ? "button" : (tag === "a" ? "link" : "text");
              const readabilityPayload = {
                foreground_rgb: fg.map(v => Math.max(0, Math.min(255, Math.round(v)))),
                background_rgb: bg.map(v => Math.max(0, Math.min(255, Math.round(v)))),
                contrast: cr,
                target_contrast: target,
                font_size: meta.fontSize || 16,
                font_weight: meta.fontWeight || 400,
                element_type: elementTypeForMl,
                // Backend requires numeric scale input; do not send any text/HTML/URLs.
                comfort_scale: (typeof comfortScale === "number" && Number.isFinite(comfortScale)) ? comfortScale : 0.9
              };

              const mlRes = await callBackendReadability(baseUrl, readabilityPayload);

              if (mlRes.ok) {
                stats.totalMlSuccess++;
                const ml = mlRes.data;

                // WCAG already passes: ML cannot force correction.
                if (cr < target) {
                  // WCAG fails: ML may veto correction only if comfortable=true and score above threshold.
                  // Threshold is deterministic and aligned with backend semantics: comfort_score is a 0-1 probability.
                  const ML_VETO_THRESHOLD = 0.5;
                  if (ml.comfortable === true && ml.comfort_score >= ML_VETO_THRESHOLD) {
                    shouldFlag = false;
                    aiReadabilityCheck = { comfortable: true, comfort_score: ml.comfort_score, expected_contrast: ml.expected_contrast };
                    comfortScore = ml.comfort_score;
                    aiPrediction = { readable: true, confidence: ml.comfort_score };
                  }
                }
              } else {
                stats.totalMlFallback++;
              }
            }
          }
        } catch (metaError) {
            ...
            try {
              ...
              if (checkCr < effectiveTarget) {
                ...
                if (fallbackCr < effectiveTarget) {
                  ...
                  // CRITICAL: Mark as terminal failure and add to scannedElements to prevent infinite loop
                  el.setAttribute('data-ai-terminal-failure', 'true');
                  scannedElements.add(el);
  ...
  /**
   * Backend health + ML readiness cache (per page load).
   * Source of truth for whether server-based supervised ML can be used in this scan.
   */
  let _backendHealthCache = null; // { checkedAt, ok, baseUrl, status, model_loaded, ready, model_name }
  const _BACKEND_BASE_URLS = ["http://127.0.0.1:5000", "http://localhost:5000"];
  const _BACKEND_HEALTH_TTL_MS = 30_000;

  async function _fetchJsonWithTimeout(url, options = {}, timeoutMs = 800) {
  ...
  async function resolveBackendHealth({ force = false } = {}) {
    const now = Date.now();
    if (!force && _backendHealthCache && (now - _backendHealthCache.checkedAt) < _BACKEND_HEALTH_TTL_MS) {
      return _backendHealthCache;
    }

    for (const baseUrl of _BACKEND_BASE_URLS) {
      try {
        const { ok, json } = await _fetchJsonWithTimeout(`${baseUrl}/health`, { method: "GET" }, 800);
        if (ok && json && typeof json === "object") {
          _backendHealthCache = {
            ...
            ready: !!json.ready,
            model_name: typeof json.model_name === "string" ? json.model_name : "unknown"
          };
          return _backendHealthCache;
        }
      } catch (e) {
  ...
  async function persistBackendStatusToContrastState({ apiAvailable, modelReady }) {
    return new Promise((resolve) => {
      chrome.storage.local.get(["contrastState"], (result) => {
        const current = result && result.contrastState && typeof result.contrastState === "object"
          ? result.contrastState
          : {};
        const next = {
          ...current,
        ...
        chrome.storage.local.set({ contrastState: next }, () => resolve(next));
      });
    });
  }

  async function callBackendReadability(baseUrl, payload) {
    if (!baseUrl) return { ok: false, error: "no_base_url" };
    try {
      const { ok, json } = await _fetchJsonWithTimeout(
        `${baseUrl}/readability`,
      ...
      const comfortable = json.comfortable;
      const comfort_score = json.comfort_score;
      const expected_contrast = json.expected_contrast;
      if (typeof comfortable !== "boolean" || typeof comfort_score !== "number" || typeof expected_contrast !== "number") {
        return { ok: false, error: "invalid_schema" };
      }
      return { ok: true, data: { comfortable, comfort_score, expected_contrast } };
    } catch (e) {
  ...
  async function scanWithAI(comfortScale = null, autoCorrect = null) {
    ...
    try {
      ...
      // Resolve backend ML availability ONCE per scan (no per-element availability checks)
      const health = await resolveBackendHealth();
      const apiAvailable = !!(health && health.ok && health.status === "ok");
      const modelReady = !!(apiAvailable && health.model_loaded === true && health.ready === true);

      await persistBackendStatusToContrastState({ apiAvailable, modelReady });

      if (modelReady) {
        console.log(`✅ [ML BACKEND] Backend reachable. Model ready: true (model=${health.model_name}). Mode=ML`);
      } else {
        console.warn(`⚠️  [ML BACKEND] Backend ML unavailable for this scan. Mode=WCAG (reachable=${apiAvailable}, model_loaded=${!!health.model_loaded}, ready=${!!health.ready})`);
      }

      // On-device correction path is unchanged (CIELAB optimization); ML is prediction-only.
      ...
      // ML does NOT influence target selection in this architecture. Target remains WCAG/comfort derived.
      const effectiveTargetForScan = target;
      const mlAvailableForScan = modelReady;
      const backendBaseUrlForScan = modelReady ? health.baseUrl : null;
      window.__aiBackendBaseUrlForScan = backendBaseUrlForScan;
      ...
      for (let sectionIdx = 0; sectionIdx < sectionsSorted.length; sectionIdx++) {
        ...
        try {
          // Get section info (background detection data - READ ONLY, not modified)
          const sectionInfo = sectionBackgroundCache.get(section);
          const rect = section.getBoundingClientRect();
          const visualTop = Math.round(rect.top + window.scrollY);

          console.log(`\n📦 Processing Section ${sectionIdx + 1}/${sectionsSorted.length} (Y: ${visualTop}px, tag: ${section.tagName.toLowerCase()})`);
          ...
          // STEP 4: Get all elements within this section (sorted by visual position)
          const sectionElements = getElementsInSection(section);
          ...
          // STEP 5: Process elements SEQUENTIALLY in strict visual order (top to bottom)
          // DETERMINISTIC: Each element is fully processed before moving to the next
          // This ensures the first detected issue is always fixed first
          // PERFORMANCE: Process in batches with yielding to prevent blocking
          const SCAN_BATCH_SIZE = 10; // Process max 10 elements per batch
          const SCAN_TIME_BUDGET_MS = 16; // ~1 frame budget per batch
          
          for (let elIdx = 0; elIdx < sectionElements.length; elIdx++) {
            const el = sectionElements[elIdx];
            
            // Yield control every SCAN_BATCH_SIZE elements or when time budget exceeded
            if (elIdx > 0 && (elIdx % SCAN_BATCH_SIZE === 0)) {
              const batchStartTime = performance.now();
              // Yield to browser to prevent blocking user interaction
              await new Promise(resolve => requestAnimationFrame(resolve));
              
              // Performance guard: If batch took too long, yield again
              const batchTime = performance.now() - batchStartTime;
              if (batchTime > SCAN_TIME_BUDGET_MS) {
            ...
            try {
              ...
              scanProgress.processed++;
              updateScanProgress(scanProgress.processed, scanProgress.total);
              // Continue to next element instead of stopping
            }
          }
        } catch (sectionError) {
      ...
      orphanElements.sort((a, b) => {
        try {
          ...
          const rectB = b.getBoundingClientRect();
          // Primary: Y-coordinate (top to bottom)
          if (Math.abs(rectA.top - rectB.top) > 3) {
            return rectA.top - rectB.top;
          }
      ...
      // DETERMINISTIC: Process orphan elements SEQUENTIALLY in strict visual order
      // PERFORMANCE: Process in batches with yielding to prevent blocking
      if (orphanElements.length > 0) {
        const ORPHAN_BATCH_SIZE = 10; // Process max 10 elements per batch
        const ORPHAN_TIME_BUDGET_MS = 16; // ~1 frame budget per batch
        
        for (let elIdx = 0; elIdx < orphanElements.length; elIdx++) {
          const el = orphanElements[elIdx];
          
          // Yield control every ORPHAN_BATCH_SIZE elements or when time budget exceeded
          if (elIdx > 0 && (elIdx % ORPHAN_BATCH_SIZE === 0)) {
            const batchStartTime = performance.now();
            // Yield to browser to prevent blocking user interaction
            await new Promise(resolve => requestAnimationFrame(resolve));
            ...
            // Performance guard: If batch took too long, yield again
            const batchTime = performance.now() - batchStartTime;
            if (batchTime > ORPHAN_TIME_BUDGET_MS) {
              await new Promise(resolve => setTimeout(resolve, 0));
            }
          }
          ...
          try {
            ...
            console.error(`❌ Error processing orphan element ${el.tagName}:`, elementError);
            stats.errors++;
            scanProgress.processed++;
            updateScanProgress(scanProgress.processed, scanProgress.total);
          }
      ...
      console.log(`   📊 Processed ${sectionsSorted.length} sections in strict visual order (top to bottom)`);
      ...
      console.log("=".repeat(80) + "\n");
      console.log(`📈 [ML BACKEND] Readability calls: total=${stats.totalMlCalls}, success=${stats.totalMlSuccess}, fallback=${stats.totalMlFallback}`);
  ...
  function applyHoverLogic(elements, targetContrast = null) {
    ...
    elements.forEach((el) => {
      // Skip only if we have a confirmed active binding (attributes can be stale after DOM replacements).
      const _boundV = el.getAttribute("data-ai-hover-bind-v");
      const _hasHandlers =
        typeof el._aiHoverInExtended === "function" &&
        typeof el._aiHoverOutExtended === "function";
      if (_boundV === String(_AI_HOVER_BIND_VERSION) && _hasHandlers) {
        return;
      }
      ...
      // Links/buttons: bind ONLY the hover+11 enhancer and skip the legacy hover-calc pipeline.
      // This keeps hover behavior non-intrusive and prevents conflicting hover systems.
      if (_aiIsLinkOrButton(el)) {
        try {
          // Remove existing handlers if any to avoid duplicates.
          if (typeof el._aiHoverInExtended === "function") {
            el.removeEventListener("mouseenter", el._aiHoverInExtended);
          }
          if (typeof el._aiHoverOutExtended === "function") {
          ...
          const hoverIn = () => {
            ...
            // Apply hover enhancement with small delay to debounce rapid mouse movements
            hoverInTimer = setTimeout(() => {
              if (el && document.contains(el)) {
                _aiApplyHoverContrastEnhancement(el, 11);
              }
              hoverInTimer = null;
            }, 10); // 10ms debounce - fast enough to feel instant, slow enough to prevent flicker
          };
          ...
          const hoverOut = () => {
            ...
            // Defer restore by one frame to prevent flicker (replaces setTimeout debounce)
          ...
          el.setAttribute("data-ai-hover-bind-v", String(_AI_HOVER_BIND_VERSION));
        } catch (_) {}
        return;
      }
      ...
      try {
        ...
        const hoverIn = async () => {
          ...
          // Hover-only 11:1 enhancement for links/buttons.
          // If current hover contrast already >= 11:1, do nothing (preserve site hover).
          // If below, apply minimal darkening of NORMAL background + adjust text, and restore on hover-out.
          if (_aiIsLinkOrButton(el)) {
            const res = _aiApplyHoverContrastEnhancement(el, 11);
            el._aiHoverActive = false;
            // Always return for links/buttons to avoid legacy hover overrides/log spam.
            return;
          }
          ...
          // 🔍 DEBUG: Log detailed state only if debug flag is enabled
          if (window._aiDebugHover) {
          ...
          // Detailed hover calculation logs only in debug mode
          if (window._aiDebugHover) {
          ...
          // Safety check passed - log only in debug mode
          if (window._aiDebugHover) {
          ...
          // Guard: Never inject hover background for links or buttons; prefer text-only correction.
          // Also forbid background changes when background is inherited/transparent.
          const _tag = (el && el.tagName ? el.tagName.toLowerCase() : "");
          const _isButton = _tag === "button" || el.getAttribute("role") === "button";
          const _isLink = _tag === "a" && !_isButton;
          const _hoverBgParsed = hoverBg ? parseCSSColorToRGBA(hoverBg, [0, 0, 0, 0]) : null;
          const _hoverBgAlpha = _hoverBgParsed ? _hoverBgParsed[3] : 0;
          const _bgChangeForbidden = _isButton || _isLink || !hasExplicitBackground(el) || _hoverBgAlpha < 0.05;

          if (hoverBg && hasExplicitBackground(el) && !_bgChangeForbidden) {
        ...
        const hoverOut = async () => {
          ...
          // Store original transition for later restore
          const originalTransition = el.style.transition || getComputedStyle(el).transition;

          // Defer restoration by one frame to prevent flicker
          requestAnimationFrame(() => {
            if (!el || !document.contains(el)) {
              el._aiHoverRestoring = false;
              return;
  ...
  async function initHoverCorrection(targetContrast = null) {
    ...
    // Apply hover-only enhancement strictly to links and buttons.
    // NOTE: do not exclude by data-hover-bound/data-ai-contrast-fixed here; those attributes can be stale
    // when frameworks replace DOM nodes. applyHoverLogic() will skip only if handlers are actually bound.
    const allTargets = document.querySelectorAll("button, a, [role=\"button\"]");
    ...
    hoverObserver = new MutationObserver((mutations) => {
          ...
          for (const node of mutation.addedNodes) {
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              const hasInteractiveChildren =
                ...
                node.querySelectorAll('button, a, [role="button"]').length > 0;
      ...
      if (hasNewInteractiveElements) {
        ...
        debounceTimer = setTimeout(() => {
          ...
          // FIX: Also exclude elements with data-ai-contrast-fixed to prevent duplicate hover listeners
          const newTargets = document.querySelectorAll('button, a, [role="button"]');
  ...
  function getMLPredictedContrast(currentSettings) {
    ...
    return Promise.resolve(currentSettings.targetContrast);
  }
  ...
  function _clearElementCorrections(el) {
    ...
    el.removeAttribute('data-ai-corrected-color');
    ...
    el.removeAttribute('data-correction-reason');
    el.removeAttribute('title');
    
    // Remove from scanned elements set so it can be re-processed
    if (typeof scannedElements !== 'undefined' && scannedElements.delete) {
      scannedElements.delete(el);
    }
    
    // Clear inline style color (will be recalculated)
    if (el.style) {
      el.style.removeProperty('color');
    }
  }

  // Helper function to wait for CSS animations/transitions and jQuery animations to complete
  // Returns a promise that resolves when animations are done
  // Uses polling to detect jQuery animations from custom plugins
  function waitForAnimations(el, maxWait = 500) {
    return new Promise((resolve) => {
      if (!el || !document.contains(el)) {
        resolve();
        return;
      ...
      let lastJqState = false;
      let stableCount = 0;
      const STABLE_THRESHOLD = 3; // Require 3 consecutive checks with no animations

      // Polling function to check for jQuery animations
      function checkJQueryAnimations() {
        ...
        try {
          ...
          // Check element and all ancestors for jQuery animations
          const hasAnimation = $el.is(':animated') || 
                              $el.parents(':animated').length > 0 ||
                              // Also check for jQuery queue (custom animations)
                              ($el.queue && $el.queue().length > 0) ||
                              // Check parent elements' queues too
                              ($el.parent().queue && $el.parent().queue().length > 0);
      ...
      function checkCSSAnimations(element) {
        ...
        try {
          ...
          return (transitionDuration > 0 && transitionDuration < 10) || 
                 (animationDuration > 0 && animationDuration < 10);
        } catch (e) {
          return false;
        }
      }
      ...
      function poll() {
        ...
        // Check jQuery animations (polling approach)
        const hasJqAnimation = checkJQueryAnimations();
        
        // Check CSS animations on element and parents
        let hasCSSAnimation = checkCSSAnimations(el);
        if (!hasCSSAnimation) {
          let parent = el.parentElement;
          let depth = 0;
          while (parent && parent !== document.body && depth < 5) {
            if (checkCSSAnimations(parent)) {
        ...
        const hasAnyAnimation = hasJqAnimation || hasCSSAnimation;

        if (hasAnyAnimation) {
          stableCount = 0; // Reset stable count
          lastJqState = hasJqAnimation;
          // Continue polling
          requestAnimationFrame(poll);
        } else {
          stableCount++;
          if (stableCount >= STABLE_THRESHOLD) {
            // Animations have been stable for multiple checks - resolve
            setTimeout(resolve, 20); // Small delay to ensure browser processed changes
          } else {
  ...
  // Track elements currently being processed to prevent infinite loops
  const elementsBeingProcessed = new Map(); // Map<Element, Promise>
  
  // NON-ENFORCEABLE ELEMENTS: Track elements whose fixes cannot be enforced due to site CSS overrides
  const nonEnforceableElements = new WeakSet(); // Set<Element>
  
  // LOGGING GUARDS: Track logged events per element to prevent log spam
  const loggedHoverIn = new WeakSet(); // Elements that logged hover in
  const loggedHoverOut = new WeakSet(); // Elements that logged hover out
  const loggedApplyFailure = new WeakSet(); // Elements that logged apply failure
  
  // A) MUTATION FILTERING: Operation token based tracking (replaces time-based unmarking)
  const activeOperations = new WeakMap(); // Map<Element, Set<operationId>>
  const extensionOwnedAttributes = new WeakMap(); // Track which attributes we set
  
  // Generate unique operation ID
  let operationIdCounter = 0;
  function generateOperationId() {
    return `op-${Date.now()}-${++operationIdCounter}-${Math.random().toString(36).substr(2, 9)}`;
  }
  
  // Mark element with operation token
  function markElementOperation(el, operationId) {
    if (!el) return;
    const ops = activeOperations.get(el) || new Set();
    ops.add(operationId);
    activeOperations.set(el, ops);
    
    // DEBUG: Track operation token creation
    if (window._aiDebugMutations && typeof debugStats !== 'undefined') {
      debugStats.operationTokensCreated++;
    }
  }
  
  // Unmark element operation
  function unmarkElementOperation(el, operationId) {
    if (!el) return;
    const ops = activeOperations.get(el);
    if (ops) {
      ops.delete(operationId);
      if (ops.size === 0) {
        activeOperations.delete(el);
      }
      
      // DEBUG: Track operation token clearing
      if (window._aiDebugMutations && typeof debugStats !== 'undefined') {
        debugStats.operationTokensCleared++;
      }
    }
  }
  
  // Check if element has active operations
  function hasActiveOperations(el) {
    if (!el) return false;
    const ops = activeOperations.get(el);
    return ops && ops.size > 0;
  }
  
  // C) ORPHAN RULE CLEANUP: Remove stylesheet rules for disconnected elements
  function cleanupOrphanRules(el) {
    if (!el) return;
    
    const fixId = el.getAttribute('data-ai-colorfix-id');
    if (!fixId) return;
    
    try {
      const contrastSheet = document.getElementById('contrast-fixes');
      if (!contrastSheet || !contrastSheet.sheet) return;
      ...
      for (let i = rules.length - 1; i >= 0; i--) {
        const rule = rules[i];
        if (rule.selectorText && rule.selectorText.includes(fixId)) {
          sheet.deleteRule(i);
          cleaned++;
        }
      }
      ...
      if (cleaned > 0) {
        if (window._aiDebugMutations) {
          debugStats.orphanCleanups += cleaned;
          console.log(`   🧹 [CLEANUP] Removed ${cleaned} orphan rule(s) for fixId: ${fixId}`);
        }
      }
    } catch (e) {
      // Silently fail - element may be gone
    }
  }

  // Robust function to ensure a color fix persists, with multiple retries
  async function ensureFixPersists(el, property, expectedValue, maxRetries = 3) {
    if (!el || !document.contains(el)) return false;

    // CRITICAL: Prevent infinite loops - check if already processing this element
    const existingPromise = elementsBeingProcessed.get(el);
    if (existingPromise) {
      // Already processing - wait for existing promise to complete
      return existingPromise;
    }

    const expectedMatch = expectedValue.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/i);
    if (!expectedMatch) return false; // Can't verify non-RGB values
    ...
    const processPromise = (async () => {
      const operationId = generateOperationId();
      try {
        // C) DISCONNECT DETECTION: Check if element is still connected
        if (!document.contains(el)) {
          // Element was replaced/disconnected - cleanup orphan rules
          cleanupOrphanRules(el);
          // DEBUG: Track disconnect detection
          if (window._aiDebugMutations && typeof debugStats !== 'undefined') {
            debugStats.disconnectDetections++;
          }
          return false;
        }
        
        // First, check if fix is already correct BEFORE applying anything
        await waitForAnimations(el, 200);
        
        // Check again after animation wait
        if (!document.contains(el)) {
          cleanupOrphanRules(el);
          // DEBUG: Track disconnect detection
          if (window._aiDebugMutations && typeof debugStats !== 'undefined') {
        ...
        const initialComputedColor = getComputedStyle(el).color;
        const initialMatch = initialComputedColor.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/i);
        
        if (initialMatch) {
          const initialRgb = [parseInt(initialMatch[1]), parseInt(initialMatch[2]), parseInt(initialMatch[3])];
          const alreadyMatches = expectedRgb.every((val, i) => Math.abs(val - initialRgb[i]) <= 1);
          
          if (alreadyMatches) {
            // Fix is already correct - no need to do anything
            return true;
          }
        }

        for (let attempt = 0; attempt < maxRetries; attempt++) {
          // C) DISCONNECT DETECTION: Check before each retry
          if (!document.contains(el)) {
            cleanupOrphanRules(el);
            // DEBUG: Track disconnect detection
            if (window._aiDebugMutations && typeof debugStats !== 'undefined') {
          ...
          // A) OPERATION TOKEN: Mark with operation ID (not time-based)
          markElementOperation(el, operationId);
          
          // Wait for animations before each attempt
          await waitForAnimations(el, 200);
          ...
          // Apply the fix using both inline style and stylesheet injection
          applyColorWithImportant(el, property, expectedValue);
          
          // Also use forceApplyStyle as backup
          forceApplyStyle(el, property, expectedValue);
          
          // Wait for DOM to settle and verify on next frame
          await new Promise(resolve => requestAnimationFrame(resolve));
          await new Promise(resolve => setTimeout(resolve, 100 + (attempt * 50)));

          // C) DISCONNECT DETECTION: Check before verification
          if (!document.contains(el)) {
            unmarkElementOperation(el, operationId);
            cleanupOrphanRules(el);
            // DEBUG: Track disconnect detection
          ...
          if (window._aiDebugMutations && typeof debugStats !== 'undefined' && attempt > 0) {
            debugStats.retries++;
          }
          ...
          if (computedMatch) {
            const computedRgb = [parseInt(computedMatch[1]), parseInt(computedMatch[2]), parseInt(computedMatch[3])];
            const matches = expectedRgb.every((val, i) => Math.abs(val - computedRgb[i]) <= 1);
            
            if (matches) {
              // Success! Unmark and return
              unmarkElementOperation(el, operationId);
              return true;
            }
          }

          // Unmark before retry (operation token cleanup)
          unmarkElementOperation(el, operationId);
          
          // If this is not the last attempt, retry silently
          if (attempt < maxRetries - 1) {
            await new Promise(resolve => setTimeout(resolve, 100));
          }
        }
        
        // Final unmark after all retries
        unmarkElementOperation(el, operationId);
        
        // NON-ENFORCEABLE GUARD: Mark element as non-enforceable if all retries failed
        // This prevents repeated reprocessing of elements whose fixes cannot be enforced
        if (!nonEnforceableElements.has(el)) {
          nonEnforceableElements.add(el);
          console.warn(`⚠️ [NON-ENFORCEABLE] Element ${el.tagName}${el.id ? `#${el.id}` : ''}${el.className ? `.${el.className.split(' ')[0]}` : ''} cannot have contrast fix enforced after ${maxRetries} attempts. Skipping future retries.`);
        }
        ...
        // Remove from processing map (cleanup via microtask)
        Promise.resolve().then(() => {
          elementsBeingProcessed.delete(el);
        });
      }
    })();

    // Store the promise
    elementsBeingProcessed.set(el, processPromise);
  ...
  function startObservingDynamicContent() {
    ...
    // Enable MutationObserver with optimized conditions for fast incremental fixes
    let debounceTimer = null;
    let isScanning = false; // Flag to prevent multiple simultaneous rescans
    let lastScanTime = 0; // Track when last scan completed
    const DEBOUNCE_MS = 100; // Debounce window for batching mutations
    const MIN_SCAN_INTERVAL = 100; // Fast minimum interval (100ms) for incremental fixes
    const FULL_RESCAN_THRESHOLD = 20; // If more than 20 elements changed, do full rescan instead
    const BATCH_SIZE = 15; // Process max 15 elements per tick to avoid blocking
    
    // Track elements that need processing (for incremental fixes)
    const elementsToProcess = new Set();
    let processingTimer = null;
    let rafScheduled = false; // Track if RAF processing is scheduled
    
    // DEBUG: Structured logging (enable via window._aiDebugMutations = true)
    const debugStats = {
      mutationsObserved: 0,
      skippedAsOwn: 0,
      queued: 0,
      ...
      retries: 0,
      orphanCleanups: 0,
      disconnectDetections: 0,
      operationTokensCreated: 0,
      operationTokensCleared: 0
    };
    
    // Expose debug stats
    if (typeof window !== 'undefined') {
      window._aiMutationStats = debugStats;
      
      // Add helper function to log stats summary
      window._aiLogMutationStats = function() {
        console.log('📊 [MUTATION STATS]', {
          mutationsObserved: debugStats.mutationsObserved,
          skippedAsOwn: debugStats.skippedAsOwn,
          queued: debugStats.queued,
          processed: debugStats.processed,
          retries: debugStats.retries,
          orphanCleanups: debugStats.orphanCleanups,
          disconnectDetections: debugStats.disconnectDetections,
          operationTokensCreated: debugStats.operationTokensCreated,
          operationTokensCleared: debugStats.operationTokensCleared,
          activeTokensRemaining: debugStats.operationTokensCreated - debugStats.operationTokensCleared
        });
      };
    }
    
    // A) MUTATION FILTERING: Check if mutation is from our own changes (operation token based)
    function isOurOwnMutation(mutation) {
      const target = mutation.target;
      if (!target) return false;
      
      // Check if element has active operations (token-based, not time-based)
      if (hasActiveOperations(target)) {
        return true;
      }
      
      // Check if element is being processed
      if (elementsBeingProcessed.has(target)) {
        return true;
      }
      
      // Note: ai-color-override classes are no longer created or used.
      // All mutation filtering is now handled by operation tokens and activeOperations tracking.
      
      // For style mutations: check if we own this attribute change
      if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
        const ownedAttrs = extensionOwnedAttributes.get(target);
        if (ownedAttrs && ownedAttrs.has('style')) {
          return true;
        }
      }
    ...
    // Function to process elements incrementally (fast)
    async function processChangedElementsIncrementally() {
      if (elementsToProcess.size === 0) {
        return;
      }
      
      if (isScanning) {
        console.log(`   ⏸️ [INCREMENTAL] Skipping - scan in progress (${elementsToProcess.size} elements queued)`);
        return;
      }
      
      if (window._aiDebugMutations) {
        console.log(`   🚀 [INCREMENTAL] Starting processing for ${elementsToProcess.size} element(s)`);
      }
      
      // Get settings once
      const settings = await getCurrentSettings();
      if (!settings.autoCorrect) {
        console.log(`   ⏸️ [INCREMENTAL] Auto-correct disabled, clearing queue`);
        elementsToProcess.clear();
        return;
      }
      
          // If too many elements, trigger full rescan instead of incremental processing
          // But only if we're not already processing (avoid race conditions)
          if (elementsToProcess.size > FULL_RESCAN_THRESHOLD) {
            const elementsCount = elementsToProcess.size;
            elementsToProcess.clear();
            // Trigger full rescan by calling scanWithAI directly
            // This is more efficient than waiting for debounce when we know we need a full scan
            getCurrentSettings().then(async (settings) => {
              if (!settings.autoCorrect || isScanning) return;
              isScanning = true;
              ...
              try {
                await scanWithAI(settings.comfortScale, settings.autoCorrect);
              } catch (err) {
                console.error("⚠️ Full rescan from incremental threshold failed:", err);
              } finally {
                setTimeout(() => {
      ...
      isScanning = true;
      // Calculate target contrast (same as scanWithAI)
      const target = settings.comfortScale ? settings.comfortScale * 15.5 : 8.0;
      const stats = { flagged: 0, corrected: 0, errors: 0, skipped: 0 };
      
      // B) BATCH PROCESSING: Process max BATCH_SIZE elements per tick
      // Convert Set to Array and take only BATCH_SIZE
      const allElements = Array.from(elementsToProcess);
      const elementsToProcessThisTick = allElements.slice(0, BATCH_SIZE);
      
      // Remove processed elements from the queue
      for (const el of elementsToProcessThisTick) {
        elementsToProcess.delete(el);
      }
      
      // Process elements sequentially for correctness
      for (const el of elementsToProcessThisTick) {
        ...
        try {
          ...
          // D) FIX PERSISTENCE: Ensure fix persists with capped retries
          const correctedColor = el.getAttribute('data-ai-corrected-color') || el.getAttribute('data-corrected-fg');
          if (correctedColor) {
          ...
          // Silently mark as processed to prevent retry loops
          if (el) {
            scannedElements.add(el);
          }
        }
      ...
      if (elementsToProcess.size > 0) {
        ...
        requestAnimationFrame(() => {
          ...
          processingTimer = setTimeout(() => {
            ...
            processChangedElementsIncrementally();
          }, DEBOUNCE_MS);
        });
      }
      
      // Only log if we actually fixed something (reduce console noise)
      if (stats.corrected > 0) {
        console.log(`   ⚡ [INCREMENTAL] Fixed ${stats.corrected} element(s) in batch (${elementsToProcess.size} remaining)`);
      }
    }
    ...
    mutationObserver = new MutationObserver((mutations) => {
      ...
      // DEBUG: Count mutations observed
      if (window._aiDebugMutations) {
        debugStats.mutationsObserved += mutations.length;
      }
      
      for (const mutation of mutations) {
        // A) MUTATION FILTERING: Skip our own mutations
        if (isOurOwnMutation(mutation)) {
          // DEBUG: Count skipped
          if (window._aiDebugMutations) {
            debugStats.skippedAsOwn++;
          }
          continue; // Ignore mutations we caused
        }
        ...
        if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
          ...
          // Also check if the style change matches our expected corrected color
          // If it does, it's likely our own change and we should ignore it
          if (target && (target.hasAttribute('data-contrast-fixed') || target.hasAttribute('data-ai-contrast-fixed'))) {
            const correctedColor = target.getAttribute('data-ai-corrected-color') || 
                                   target.getAttribute('data-corrected-fg');
            if (correctedColor) {
              // Check if current style matches our expected color
              const currentStyle = target.getAttribute('style') || '';
              const correctedMatch = correctedColor.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/i);
              
              if (correctedMatch && currentStyle.includes(correctedColor.replace(/\s/g, ''))) {
                // Style matches our expected color - this is our own change, ignore it
                continue;
              }
              
              // Style doesn't match - something else changed it, re-apply our fix
              // But only if not already being processed and not marked as non-enforceable
              if (!elementsBeingProcessed.has(target) && !hasActiveOperations(target) && !nonEnforceableElements.has(target)) {
        ...
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
          ...
          // E) BOUNDED SUBTREE SCAN: For tab-like containers, scan within container with caps
          const targetClasses = Array.from(target.classList).join(' ').toLowerCase();
          const isTabLikeElement = targetClasses.includes('tab') || 
                                   targetClasses.includes('active') ||
                                   target.closest('[class*="tabs"]') ||
                                   target.closest('[class*="tab-"]');
          
          // Collect elements that need re-evaluation (bounded subtree scan for tabs)
          const elementsToCheck = [];
          
          // E) BOUNDED SUBTREE: Identify tab panel container
          let container = null;
          if (isTabLikeElement) {
            // Find nearest tab panel container
            container = target.closest('[class*="tabs"], [class*="tab-"], .tm_pb_tabs, .tab-content, .tab-panel');
            if (!container) {
              // Fallback: use target's parent if it looks like a tab container
              const parent = target.parentElement;
              if (parent && (parent.classList.toString().toLowerCase().includes('tab') || 
                            parent.classList.toString().toLowerCase().includes('tabs'))) {
                container = parent;
              }
            }
            // If still no container, use target itself
            if (!container) {
              container = target;
            }
          } else {
          ...
          // E) BOUNDED SCAN: Scan within container with node count and time budget caps
          const MAX_NODES = 100; // Cap node count
          const MAX_TIME_MS = 50; // Cap time budget (50ms)
          const startTime = performance.now();
          let nodeCount = 0;
          
          // Helper to check if we should continue scanning
          const shouldContinue = () => {
            return nodeCount < MAX_NODES && (performance.now() - startTime) < MAX_TIME_MS;
          };
          
          // Add target itself if relevant
          if (target.hasAttribute('data-ai-contrast-fixed') || target.hasAttribute('data-contrast-fixed')) {
            elementsToCheck.push(target);
            nodeCount++;
          }
          
          const targetTag = target.tagName ? target.tagName.toLowerCase() : '';
          const isTargetInteractive = targetTag === 'a' || targetTag === 'button' || 
                                     target.getAttribute('role') === 'button' || 
                                     target.getAttribute('role') === 'link';
          
          if (isTargetInteractive && !elementsToCheck.includes(target)) {
            elementsToCheck.push(target);
            nodeCount++;
          }
          
          // Bounded subtree scan within container
          if (container && shouldContinue()) {
            const walker = document.createTreeWalker(
              ...
              NodeFilter.SHOW_ELEMENT,
              {
                acceptNode: (node) => {
                  ...
                  if (isInteractive || 
                      node.hasAttribute('data-ai-contrast-fixed') || 
                      node.hasAttribute('data-contrast-fixed')) {
            ...
            let node;
            while ((node = walker.nextNode()) && shouldContinue()) {
              if (!elementsToCheck.includes(node)) {
          ...
          if (elementsToCheck.length > 0) {
            // Add elements to queue (don't process immediately)
            for (const el of elementsToCheck) {
              if (!el || !document.contains(el) || hasActiveOperations(el) || nonEnforceableElements.has(el)) continue;
            ...
            if (node.nodeType === Node.TEXT_NODE) {
              ...
              if (text.length > 0) {
                ...
                if (parent && parent !== document.body) {
                  ...
                  // Add parent to incremental processing instead of full rescan
                  if (parent && document.contains(parent) && !nonEnforceableElements.has(parent)) {
              ...
              if (isInteractive) {
                // For interactive elements, check if they or immediate ancestors have solid background
                try {
                  const elCs = getComputedStyle(node);
                  const elBg = parseCSSColorToRGBA(elCs.backgroundColor, [0, 0, 0, 0]);
                  let hasSolidBg = elBg[3] > 0.5;
                  
                  // If element doesn't have solid bg, check immediate ancestors
                  if (!hasSolidBg) {
                    let checkAncestor = node.parentElement;
                    let checkDepth = 0;
                    while (checkAncestor && checkAncestor !== document.body && checkDepth < 5) {
                      try {
                        const ancestorCs = getComputedStyle(checkAncestor);
                        const ancestorBgImage = ancestorCs.backgroundImage;
                        if (ancestorBgImage && ancestorBgImage !== 'none' && ancestorBgImage.includes('url(') && !ancestorBgImage.includes('gradient')) {
                          break; // Stop at image background
                        }
                        const ancestorBg = parseCSSColorToRGBA(ancestorCs.backgroundColor, [0, 0, 0, 0]);
                        if (ancestorBg[3] > 0.5) {
                          hasSolidBg = true;
                  ...
                  if (hasSolidBg) {
                    // Interactive element with solid background - add to incremental processing
                    if (node && document.contains(node) && !nonEnforceableElements.has(node)) {
              ...
              if (isInteractive) {
                ...
                try {
                  ...
                  if (hasSolidBg) {
                    // Interactive element with solid background - add to incremental processing
                    // Timer will be set by batch processing at end of mutation callback
                    if (node && document.contains(node) && !nonEnforceableElements.has(node)) {
                      elementsToProcess.add(node);
                    }
                    break;
              ...
              if (!isInsideImageBg && !isAlreadyProcessed) {
                // Add to incremental processing queue instead of full rescan
                if (node && document.contains(node) && !nonEnforceableElements.has(node)) {
                  elementsToProcess.add(node);
                }
                break;
      ...
      // B) DEBOUNCED BATCHING: Schedule processing with RAF + debounce
      if (elementsToProcess.size > 0 && !isScanning && !rafScheduled) {
        rafScheduled = true;
        
        // Use requestAnimationFrame for next frame, then debounce
        requestAnimationFrame(() => {
          ...
          // Only trigger full rescan for large changes (not handled by incremental processing)
          // Skip if we're already processing incrementally or if changes are small
          if (hasRelevantChanges && !isScanning && elementsToProcess.size === 0) {
            // Check settings early to avoid unnecessary processing
            const now = Date.now();
            const timeSinceLastScan = now - lastScanTime;
            
            // Skip if we scanned too recently (respect MIN_SCAN_INTERVAL)
            if (timeSinceLastScan < MIN_SCAN_INTERVAL) {
              if (debounceTimer) {
                clearTimeout(debounceTimer);
                debounceTimer = null;
              }
              const remainingTime = MIN_SCAN_INTERVAL - timeSinceLastScan;
              debounceTimer = setTimeout(() => {
                debounceTimer = null;
              }, remainingTime + DEBOUNCE_MS);
              return;
            }
            ...
            getCurrentSettings().then(settings => {
              ...
              debounceTimer = setTimeout(async () => {
                if (isScanning || elementsToProcess.size > 0) {
                  return; // Skip if incremental processing is handling it
                }
                ...
                console.log("🔄 Large content change detected, full rescanning...");
                isScanning = true;
                ...
                try {
                  await scanWithAI(currentSettings.comfortScale, currentSettings.autoCorrect);
                } catch (err) {
